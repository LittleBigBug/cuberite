...
void cProtocol_1_10_0::WriteEntityMetadata(cPacketizer & a_Pkt, const cEntity & a_Entity) const
{
	...
	switch (a_Entity.GetEntityType())
	{
		...
		case cEntity::etArmorStand:
		{
			...
			if (!ArmorStand.IsMarker())  // No point sending orientation if it's a marker (Marker type is punctual like a floating particle). However invisible armor stand keep armor visible according to orientations.
			{
